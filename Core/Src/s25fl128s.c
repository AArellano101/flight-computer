...
/**
  ...
  * @retval QSPI memory status
  */
int32_t S25FL128S_Enter4BytesAddressMode(
    QSPI_HandleTypeDef *Ctx,
    S25FL128S_Interface_t Mode)
{
    UNUSED(Ctx);
    UNUSED(Mode);

    /*
     * S25FL128L does not require global 4-byte mode here.
     * This driver uses fixed 4-byte-address instructions.
     */
    return S25FL128S_OK;
}
...
int32_t S25FL128S_ResetEnable(QSPI_HandleTypeDef *Ctx, S25FL128S_Interface_t Mode)
{
  ...
  s_command.InstructionMode   = QSPI_INSTRUCTION_1_LINE;
  s_command.Instruction       = S25FL128S_SOFTWARE_RESET_ENABLE_CMD; // changed
  s_command.AddressMode       = QSPI_ADDRESS_NONE;
  s_command.AlternateByteMode = QSPI_ALTERNATE_BYTES_NONE;
  s_command.DummyCycles       = 0;
  s_command.DataMode          = QSPI_DATA_NONE;
